...
var spro_slow_log_at_end = false;
jQuery(document).ready(function($) {
...
//    row.remove();
    ...
    $('.spro_slow_queries').scroll(function() {
		if (spro_slow_log_at_end) return;
        if ($(this).scrollTop() + $(this).innerHeight() >= ($(this)[0].scrollHeight - 20)) {
			if (!fetching) {
				...
				$.post(
					...
					function(response){
						if (jQuery(response).find('tr').length < 10) {
							spro_slow_log_at_end = true;
						}
	...
	$('#spro-slow-export-csv-button').click(function(e) {
        e.preventDefault();
		var nonce = $(this).data('nonce');
        window.location.href = sproVars.ajaxUrl + '?action=spro_slow_export_to_csv&nonce=' + nonce;
    });
});
jQuery(document).ready(function($) {
...
jQuery(document).ready(function($) {
    $('#clear_author_count_cache').click(function(e) {
		console.log('clearing author count cache');
        e.preventDefault();
        ...
        $.ajax({
            ...
            url: sproVars.ajaxUrl,
            data: {
                action: 'spro_clear_authorcount_cache',
                nonce: sproVars.nonce
            },
            success: function(response) {
                alert(response);
...
jQuery(document).ready(function($) {
	...
	$('#spro_recount_now').click(function(e) {
		e.preventDefault();
		$.ajax({
            url: sproVars.ajaxUrl,
            type: 'post',
            data: {
                action: 'spro_recount_terms'
            },
            success: function(response) {
				console.log(response);
                alert('Rows recounted');
            }
        });
	});
});
